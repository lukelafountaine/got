...
ava_1.default('extend merges URL instances', t => {
    // @ts-ignore Custom instance.
    const a = source_1.default.extend({ custom: new url_1.URL('https://example.com') });
    // @ts-ignore Custom instance.
...
ava_1.default('only plain objects are freezed', with_server_1.default, async (t, server, got) => {
    server.get('/', echoHeaders);
    const instance = got.extend({
        agent: new http_1.Agent({ keepAlive: true })
    });
