...
ava_1.default.serial('does not retry after cancelation', with_server_1.withServerAndLolex, async (t, server, got, clock) => {
    ...
    await t.throwsAsync(gotPromise, source_1.CancelError);
    await t.notThrowsAsync(promise, 'Request finished instead of aborting.');
});
ava_1.default.serial('cancels in-progress request', with_server_1.withServerAndLolex, async (t, server, got, clock) => {
    const { emitter, promise } = prepareServer(server, clock);
...
ava_1.default.serial('cancels in-progress request with timeout', with_server_1.withServerAndLolex, async (t, server, got, clock) => {
    const { emitter, promise } = prepareServer(server, clock);
    const body = new stream_1.Readable({
        read() { }
    });
...
ava_1.default.serial('cancel immediately', with_server_1.withServerAndLolex, async (t, server, got, clock) => {
    const promise = new Promise((resolve, reject) => {
...
ava_1.default.serial('throws on incomplete (canceled) response - promise', with_server_1.withServerAndLolex, async (t, server, got, clock) => {
...
ava_1.default.serial('throws on incomplete (canceled) response - promise #2', with_server_1.withServerAndLolex, async (t, server, got, clock) => {
    server.get('/', downloadHandler(clock));
...
ava_1.default.serial('throws on incomplete (canceled) response - stream', with_server_1.withServerAndLolex, async (t, server, got, clock) => {
    server.get('/', downloadHandler(clock));
