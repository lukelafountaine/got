...
ava_1.default('works', with_server_1.default, async (t, server) => {
    ...
    const error = await t.throwsAsync(source_1.default.get(`${server.url}/404`), source_1.HTTPError);
    t.is(error.response.body, 'not found');
    await t.throwsAsync(source_1.default.get('.com', { retry: 0 }), 'Invalid URL: .com');
});
